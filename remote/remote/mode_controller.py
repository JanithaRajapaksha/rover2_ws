...
class UDPJoystick(Node):
    ...
    def receive_data(self):
        try:
            ...
            try:
                ...
                x = float(parts[1])
                y = float(parts[2])
                speed_btn = int(float(parts[3]))
                cruise_btn = int(float(parts[4]))
            ...
            if self.current_mode in ["manual", "cruise"]:
                ...
                twist.linear.y = -self.last_lin_y * self.linear_scale
                if self.speed_level == 4:
                    twist.angular.z = self.last_y * -0.75
                else:
    ...
    # def _kill_follow_nodes(self):
    #     """Kill all processes related to follow mode."""
    #     patterns = [
    ...
    #         try:
    #             subprocess.run(["pkill", "-f", p], check=False)
    #             self.get_logger().info(f"Killed follow node: {p}")
    #         except Exception as e:
    #             self.get_logger().error(f"Failed to kill follow node {p}: {e}")
    ...
    def _launch_follow_process(self):
        self.follow_proc = subprocess.Popen(
    ...
    def _kill_follow_launch(self):
        try:
            if hasattr(self, "follow_proc") and self.follow_proc.poll() is None:
                self.get_logger().info("Stopping old follow launch...")
                self.follow_proc.terminate()
                time.sleep(0.5)

                if self.follow_proc.poll() is None:
                    self.get_logger().info("Force killing follow launch...")
                    self.follow_proc.kill()

                time.sleep(0.3)
        except Exception as e:
            self.get_logger().error(f"Failed to stop follow launch: {e}")


def _kill_follow_launch(self):
    """Fully kill everything created by follow-mode launch."""

    self.get_logger().info("Stopping follow mode processes...")

    # Kill ROS 2 nodes spawned by launch file
    patterns = [
            "person_tracker_pid_node",
            "tof_pid_node",
            "cmd_mux_node_follow",
    ...
    for p in patterns:
        try:
            subprocess.run(["pkill", "-f", p], check=False)
            self.get_logger().info(f"Killed: {p}")
        except Exception as e:
            self.get_logger().error(f"Failed to kill {p}: {e}")

    # Also kill the launch process if still alive
    try:
        if hasattr(self, "follow_proc") and self.follow_proc.poll() is None:
            self.get_logger().info("Stopping follow launch main process...")
            self.follow_proc.terminate()
            time.sleep(0.3)
            if self.follow_proc.poll() is None:
                self.get_logger().info("Force killing follow launch main process...")
                self.follow_proc.kill()
    except Exception as e:
        self.get_logger().error(f"Launch kill error: {e}")
